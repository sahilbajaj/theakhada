-- Player of the Week / Player of the Month: rank by Bayesian-shrunk win rate
-- instead of raw win rate, so volume of matches counts.
--
-- Score = (wins + K/2) / (matches + K), where K = 5 (prior of 0.5 win rate).
-- A 3-0 player scores 0.69; a 20-2 player scores 0.83 and wins.
--
-- Only the ORDER BY inside window_week and window_month changes; the rest of
-- get_club_insights is unchanged.
...
begin
  ...
  window_week as (
    ...
    having count(*) >= 3
    order by ((count(*) filter (where pm.is_win))::numeric + 2.5)
             / (count(*)::numeric + 5) desc,

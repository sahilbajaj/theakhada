-- Rework points formula: anchor the rating prior at 3.0 with scale 1.0
-- (default rating contributes 0 pts, higher/lower nudge up/down and fade
-- to zero by 8 matches), and shrink accumulated match points by
-- matchCount / (matchCount + 3) so thin records aren't over-weighted.
--
-- Motivation: previously a brand-new default-rated player showed up with
-- 6.0 pts (rating 3.0 * 2 * 1.0) before playing anything, which read as
-- unearned points on the Players roster and inflated their combined
-- seed. Mirrors the client formula in src/features/seeding/logic/computeSuggested.ts.
--
      ...
      scored as (
        select
          ...
          coalesce(t.total_points, 0)
            * coalesce(t.match_count, 0)::numeric
